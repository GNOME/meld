...
class DirDiff(melddoc.MeldDoc, glade.Component):
    ...
    UI_ACTIONS = (
        ...
        ('diff_menu', None, _('_Diff')),
            ('refresh', gtk.STOCK_REFRESH, None, None, ''),
            ('next_difference', gtk.STOCK_GO_DOWN, None, '<Control>D', None),
            ('previous_difference', gtk.STOCK_GO_UP, None, '<Control>E', None),
...
#                selself.treeview[i].set_model(self.model)
            for i in range(num_panes):
                self.pane[i].show()
            for i in range(num_panes, 3):
                self.pane[i].hide()
            if num_panes == 1:
...
#        self.model.clear()
        for pane, loc in misc.enumerate(locations):
            self.filecombo[pane].child.set_text(loc)
...
#        filenames = self.model.value_paths(root)
        filenames = [f.child.get_text() for f in self.filecombo[:self.num_panes]]
